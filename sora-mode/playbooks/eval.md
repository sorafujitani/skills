...
4. **Spawn N parallel candidates** on different models per the **arena** convention. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on a different model per the **arena** convention. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript under this project's `~/.claude/projects/<project-dir>/` directory. Do not glob across `~/.claude/projects/*/`; that crosses project boundaries and reads private chats from unrelated projects. Look at which files each candidate actually opened. Citing a principle is not reading its leaf file, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
