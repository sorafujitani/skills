...
4. Write the resume note off-context. Capture intent, what you were doing, progress and what's verified, current state, next steps, key files, and gotchas. Write it to a file that survives the trigger: for compaction the session scratchpad works, for going offline use a durable spot like `<repo>/.claude/plans/<slug>-resume.md` (gitignored or committed with the wip). The in-context plan won't survive summarization. If a show-me-your-work trail exists, point at it instead of duplicating it.
