...
1. Capture the live signal on the matching surface via **control**: a CPU profile for a spinning process, a heap snapshot for a leak, a CDP trace (Chrome tools) for a visual glitch. A real artifact, not a guess.
2. Reduce the artifact to the smoking gun: the function on the hot path, the retainer chain from the leaked object to a GC root, the loop firing without input. Parse large artifacts in a subagent (the **guard-the-context-window** principle), keep the reduced finding in the main thread.
